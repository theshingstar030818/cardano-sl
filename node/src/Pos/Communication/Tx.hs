...
import           Pos.Txp.Core               (TxAux (..), TxId, TxOut (..), TxOutAux (..), txaF)
...
prepareMTx getOwnUtxos hdwSigners addrs outputs addrData = do
...
submitTx
    ...
    -> AddrData m
    -> m (TxAux, NonEmpty TxOut)
submitTx getOwnUtxos enqueue ss outputs addrData = do
