...
import           Pos.Wallet.Web.ClientTypes       (CHash, CId (..), cIdToAddress)
...
convertCIdTOAddr :: (MonadWalletWebMode m) => CId a -> m Address
...
convertCIdTOAddrs :: (MonadWalletWebMode m, Traversable t) => t (CId a) -> m (t Address)
